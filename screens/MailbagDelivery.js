...
const MailbagDelivery = ({navigation, route}) => {
    ...
    const getData = () => {
        db.transaction(tx => {
          tx.executeSql("SELECT * FROM tblMailbagT CollectionOrDelivery = 'D'", [], (tx, results) => {
            var records = [];
    ...
    const onPressScan = () => {
        ...
        setTrackingNo(scannedData.toString());
    }
        ...
        try{
            ...
            await db.transaction(async(tx) => {
                await tx.executeSql(
                    ...
                    [scannedData.toString(),select,dateTime,currentLongitude,currentLatitude,remark,'D',selectedBank,device,courier],
                    (tx, results) => {
        ...
        <SafeAreaView style = {{flex:1}}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <Text style={styles.titleText}>Select Bank</Text>
                    <SelectList 
                        setSelected={(key) => setSelectedBank(key)} 
                        data={banks} 
                        save="key"
                        boxStyles={{borderRadius:16, padding:12,backgroundColor:'#fff',borderColor:'#fff',marginBottom:5,}} //override default styles
                        inputStyles={{  fontWeight: "600",textAlign:'center',fontSize:16,color:'#000'}}
...
export default MailbagDelivery;
